...
library(tidyr)
library(readr)
data_t1 <- read_csv("d_edges_t1.csv")
data_t2 <- read_csv("d_edges_t2.csv")
```
...
#1
```{r}
data1 <- data %>% select(verwendetesymptome1___1:verwendetesymptome1___93)
...
#2
```{r}
data2 <- data %>% select(verwendetesymptome2___1:verwendetesymptome2___93)
...
```{r}
data3 <- data %>% select(verwendetesymptome3___1:verwendetesymptome3___93)
...
```{r}
data4 <- data %>% select(verwendetesymptome4___1:verwendetesymptome4___93)
...
```{r}
data5 <- data %>% select(verwendetesymptome5___1:verwendetesymptome5___93)
...
```{r}
data6 <- data %>% select(verwendetesymptome6___1:verwendetesymptome6___93)
...
```{r}
data7 <- data %>% select(verwendetesymptome7___1:verwendetesymptome7___93)
...
```{r}
data8 <- data %>% select(verwendetesymptome8___1:verwendetesymptome8___93)
...
```{r}
data9 <- data %>% select(verwendetesymptome9___1:verwendetesymptome9___93)
...
```{r}
data10 <- data %>% select(verwendetesymptome10___1:verwendetesymptome10___93)
...
edges_data <- data %>% select(edge01:edge90,id)
```
...
edges_long <- edges_long %>% filter(na_check == FALSE) %>% select(-na_check) 
```
...
```{r}
werte <- data %>% select(id,wert_1:wert_93) 
```
...
```
id 8 , node "35" "Selbstverletzung"; Person did not see the item and added it. Thats why we cant get the value automatically.
Value is 760
...
Delete edges which consists oh one added node by the participant as we cannot interpret them
```{r}
  ...
  result <- tryCatch(
    {
      ...
      pecanExtract(data = data_new, id_row = i,nodes = 2:80,edges = 81:ncol(data_new),edges_sep = "_")
      NULL  
    },
    warning = function(w) {
...
```{r}
networks_rel_t1 <- list()
participant_ids <- 1:50  


for (i in participant_ids) {
 
      networks_rel_t1[[i]] <- pecanExtract(data = data_new, id_row = i,nodes = 2:80,edges = 81:ncol(data_new),edges_sep = "_")}
...
```{r}
names(networks_rel_t1) <- data_t1$record_id
```

```{r}
saveRDS(networks_rel_t1, file = "networks_rel_t1_d170325.Rda" )
```
