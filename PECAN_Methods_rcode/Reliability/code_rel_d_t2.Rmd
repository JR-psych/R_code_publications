...
```{r}
werte <- data_t1 %>% select(id,wert_1:wert_93) # braucht man fast gar nicht
```
...
for (i in participant_ids) {
      ...
      networks_rel_t2[[i]] <- pecanExtract(data = data_new, id_row = i,nodes = 2:80,edges = 81:ncol(data_new),edges_sep = "_")}
...
```{r}
saveRDS(networks_rel_t2, file = "networks_rel_t2_d170325.Rda" )
```
