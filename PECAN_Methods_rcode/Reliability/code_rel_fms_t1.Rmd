---
title: "Code Rel FMS t1 (prepare data)"
author: "JR"
date: "2025-03-30"
...
```{r}
data_id <- data %>% select(record_id,id)
```
...
#edges_long_problem <- edges_long[387,]
```
...
Drop symptoms that were not on the list and added by the participants
```{r}
...
for (i in participant_ids) {
      ...
      networks_rel_t1[[i]] <- pecanExtract(data = data_new, id_row = i,nodes = 2:74,edges = 75:ncol(data_new),edges_sep = "_")}
...
```{r}
names(networks_rel_t1) <- data$record_id
```


```{r}
saveRDS(networks_rel_t1, file = "networks_rel_t1_fms170325.Rda" )
```
